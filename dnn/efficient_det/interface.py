...
class EfficientDet(DNN):
    def __init__(self, name, load_model):
        ...
        compound_coefs = [int(i) for i in re.findall(r'\d+', self.name)]
        assert len(compound_coefs) == 1
        compound_coef = compound_coefs[0]
        ...
        if load_model:
            ...
            self.model_pth = Path(
                f"cache/efficientdet-d{compound_coef}.pth"
            )
            if not self.model_pth.exists():
                ...
                if compound_coef <= 6:
                    ...
                    ver = 2
                wget.download(
                    f"https://github.com/zylo117/Yet-Another-Efficient-Pytorch/releases/download/1.{ver}/efficientdet-d{compound_coef}.pth",
    ...
    def inference(self, image, detach=False, grad=False, dryrun = False, feature=False):
        ...
        ori_imgs, x, framed_metas = preprocess_accmpeg(image, max_size=w)
        x = torch.stack(x)
...
def preprocess_accmpeg(images, max_size):
    imgs_meta = [
        aspectaware_resize_padding(image, max_size, max_size, means=None)
        for image in images
    ]
