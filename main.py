...
import adapt.oneadapt as adapter
...
def main(command_line_args):
    ...
    for sec in tqdm(
        ...
        # get the ground truth configuration
        ...
        # get configuration, inference and examine.
        config = adapter.get_configuration(
            gt_args,app,db,command_line_args
        )
        video_name, video_config = encode(config)
        logger.info("Ours bandwidth: %d", video_config["bw"])
        video = read_video_to_tensor(video_name)
        inference(config, db, app, video_name, video_config)
        stat = examine(config, gt_args, app, db)
        ...
        # update
        adapter.update_configuration(sec, command_line_args, video, gt_video, gt_args, db, app, config, gt_video_config, gt_results)

        if command_line_args.visualize:
            vis.visualize(stat, app, video, my_results, gt_results)
